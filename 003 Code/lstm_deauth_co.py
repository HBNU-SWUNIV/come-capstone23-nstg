...
packet_data = packet_data.drop(columns=['Source','Destination'])

pd.get_dummies(packet_data['Info'])

packet_data = pd.get_dummies(packet_data, columns = ['Info'])
...
model.load_state_dict(torch.load('./lstm_30.pt'))
